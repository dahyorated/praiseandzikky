...
import { MAX_PLUS_ONES } from './names.mjs';
...
export async function loadGuests() {
  ...
  const guests = Object.entries(data ?? {})
    .map(([key, value]) => ({
      key,
      name: value?.name ?? '',
      // Records written before allowances existed simply get none.
      plusOnes: Math.min(Math.max(Number(value?.plusOnes) || 0, 0), MAX_PLUS_ONES),
